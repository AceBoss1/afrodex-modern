...
export async function cancelOrderByHash(orderHash: string): Promise<boolean> {
  ...
  const { error } = await supabase
    ...
    .or(`order_hash.eq.${orderHash},tx_hash.eq.${orderHash}`);
...
export async function deactivateOrderByHash(orderHash: string): Promise<boolean> {
  const supabase = getSupabaseClient();
  if (!supabase) {
    console.error('Cannot deactivate order - Supabase not configured');
    return false;
  }
...
export async function recordTrade(trade: TradeInput): Promise<{ success: boolean; error?: string }> {
  const supabase = getSupabaseClient();
  if (!supabase) {
    console.error('Cannot record trade - Supabase not configured');
    return { success: false, error: 'Supabase not configured' };
  }
  ...
  console.log('Trade recorded successfully! Data:', data);
  return { success: true };
}
...
export async function getTradesFromDb(baseToken: string, quoteToken: string, limit = 100): Promise<DbTrade[]> {
  ...
  console.log('Fetching trades for:', baseToken, quoteToken);
  ...
  console.log('Fetched trades from DB:', data?.length || 0);
  return data || [];
}
