...
interface TradingState {
  ...
  removeOrderByNonce: (nonce: string, userAddress: string) => void;
...
export const useTradingStore = create<TradingState>((set, get) => ({
  ...
  // CRITICAL: setOrders accepts arrays directly
  setOrders: (buy, sell) => {
  ...
  addOrder: (order, side) => {
    set((state) => {
      if (side === 'buy') {
        const orders = [...state.buyOrders, order].sort((a, b) => (b.price || 0) - (a.price || 0));
        return { buyOrders: orders };
      } else {
        const orders = [...state.sellOrders, order].sort((a, b) => (a.price || 0) - (b.price || 0));
  ...
  removeOrderByNonce: (nonce, userAddress) => {
    set((state) => ({
      buyOrders: state.buyOrders.filter(
        (o) => !(o.nonce === nonce && o.user?.toLowerCase() === userAddress.toLowerCase())
      ),
  ...
  clearTradingData: () => {
    set({ buyOrders: [], sellOrders: [], trades: [] });
  },
}));
  ...
  persist(
    (set, get) => ({
      ...
      getCustomToken: (address) => {
        return get().customTokens.find((t) => t.address.toLowerCase() === address.toLowerCase());
      },
    }),
    {
...
export const useFavoritesStore = create<FavoritesState>()(
  persist(
    (set, get) => ({
      ...
      addFavorite: (address) => {
        set((state) => {
          if (state.favorites.includes(address.toLowerCase())) return state;
      ...
      removeFavorite: (address) => {
        set((state) => ({
          favorites: state.favorites.filter((f) => f !== address.toLowerCase()),
        }));
      },
